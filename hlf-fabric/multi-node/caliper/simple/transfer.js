...
/**
 * Workload module for the 'asset' chaincode TransferAsset function.
 */
class TransferAssetWorkload extends WorkloadModuleBase {
    ...
    /**
     * Assemble TXs for TransferAsset.
     * @return {Promise<object>} The promise for the transaction payload.
     */
    async submitTransaction() {
        this.txIndex++;
        // Transfere o mesmo asset que foi criado no round 'open'
        const assetID = `asset_${this.workerIndex}_${this.txIndex}`;
        const newOwner = `Caliper_Owner_${this.workerIndex}_${this.txIndex}`;

        const args = {
            contractId: 'asset', // <--- NOME DO SEU CHAINCODE
            contractFunction: 'TransferAsset', // <--- NOME DA SUA FUNÇÃO
            contractArguments: [
                assetID,  // id
                newOwner  // newOwner
            ],
