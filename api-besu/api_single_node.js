// Configure BESU_DEPLOYER_PRIVATE_KEY, BESU_CONTRACT_ADDRESS and API_WORKERS
// through the environment; do not place secrets in source files.
...
const app = express();
const port = Number(process.env.BESU_API_PORT || '3001');
const host = process.env.API_HOST || '127.0.0.1';
app.use(express.json());

// Configuração das Variáveis de Ambiente
const DEPLOYER_PRIVATE_KEY = process.env.BESU_DEPLOYER_PRIVATE_KEY || process.env.DEPLOYER_PRIVATE_KEY;
const CONTRACT_ADDRESS = process.env.BESU_CONTRACT_ADDRESS || process.env.CONTRACT_ADDRESS;
...
app.get('/health', async (req, res) => {
    try {
        ...
        res.json({ status: 'ok', backend: 'besu', rpcUrl: BESU_RPC_URL });
    } catch (error) {
        res.status(503).json({ status: 'unavailable', backend: 'besu', error: error.message });
    }
});
...
const DOCKER_CONTAINERS_TO_MONITOR = (process.env.BESU_CONTAINERS || 'node1,node2,node3,node4,node5,node6')
...
app.listen(port, host, () => {
    ...
    console.log(`Servidor da API Besu a correr em http://${host}:${port}`);
