...
JAVA_VERSION="jdk-${JAVA_VERSION}" # Nome do diretório que o JDK será extraído
...
check_network_assets() {
    ...
    for asset in "${required[@]}"; do
        [[ -f "${BASE_DIR}/${asset}" ]] || missing+=("${asset}")
    done
    ((${#missing[@]} == 0)) || die "Artefatos Besu ausentes: ${missing[*]}. Recupere-os antes de usar BACKEND=besu."
}
